	...
	while(!b_button()){
		...
		if(on){
			...
			if(analog10(RIGHT_FRONT_SENSOR)<=right_front_blk-200&&analog10(LEFT_FRONT_SENSOR)<=left_front_blk-200)
			{
				...
				if(analog10(RIGHT_BACK_SENSOR)>=right_back_blk-200){
					turn_right() ;
					set_servo_position(SERVO,servo_pos);
					if(digital(BUTTON)){ on=(on+1)%2;msleep(100);}
					if(servo_pos>=400||servo_pos<=0){intervall*=-1; }
...
void calibrate(){
	...
	while(!a_button()){
		printf("Bitte setze die VORDEREN Sensoren auf den schwarzen Streifen\n");
	...
	while(!a_button()){
		printf("Bitte setze die HINTEREN Sensoren auf den schwarzen Streifen\n");
