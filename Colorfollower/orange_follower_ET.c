...
void main()
{
	...
	while(!b_button()){
		...
		do {
			camera_update(); 
			linefolowing();
		} while(get_object_count(orange) ==0); //linefolowing bis er was sieht
	...
	double sec=seconds();
		while(!getit){
				if((sec+10)<seconds()){ //wenn er 10 sec nichts findet 
					if(foundNothingToLong=2){
						...
						if(analog10(ET)<310){
							...
							sec=seconds();
							if(!get_object_count(orange)>0){  //wenn ers nicht hat 
								forward();
								msleep(400);
			...
			if(left_or_right){ 
				...
				while(analog10(RIGHT_SENSOR)<right_blk-150&&!get_object_count(orange)>0){ //solange Kammera und Sensor nichts sehen rechts drehen
						camera_update();
				}
				...
				while(analog10(LEFT_SENSOR)<left_blk-150&&!get_object_count(orange) >0){
						camera_update();
...
int foundit(){
	...
	if(get_object_count(orange)>0){
		while(get_object_center(orange,0).y < 100){ //solange bis blob nahe genug 
				camera_update(); 
				if(get_object_count(orange)>0){ //schaut ob Blob noch in der Kamera
					camera_update(); 
					x = get_object_center(orange,0).x;	
